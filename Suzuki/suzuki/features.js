...
function features( navigation)  {
    ...
    const [value, setValue] = useState(null);      // variant
    const [isFocus, setIsFocus] = useState(false);

    const [features_name, setFeatures_name] = useState(''); // features name

    // uplode img button 1 start
    const [imageSource, setImageSource] = useState(null);    // fatures img
...
<View style={{margin:10,padding:10}}>
    ...
    <View style={{borderColor:'', borderWidth: 0, backgroundColor:'',
            shadowColor: '',}}>
        <Text style={{ fontSize:22,  fontWeight:"900",resizeMode: 'cover', color:'blue',marginLeft:40}}> Create Suzuki Feature</Text>
    </View>
    ...
    <View style={{marginTop:20,flexDirection:'row',marginBottom:10}}>
        ...
        borderWidth: 1,
        borderRadius: 10,
        padding: 10,}}  placeholder="Enter Feature name*" placeholderTextColor='black'
        value={features_name}
        onChangeText={(text)=>setFeatures_name(text)} 
        />
...
<View style={{ flex: 1,
    ...
    marginBottom: 10,color:'black',}}>selected features img</Text>
      {imageSource && <Image source={imageSource} style={{width: 200,
    height: 200,
    resizeMode: 'contain',
    marginTop: 20,
    marginBottom:10}} />}
      <Button title=" Uplode  features Image" onPress={selectImage} />
    </View>
...
<View style={{ flexDirection: 'column',
    ...
    alignItems: 'center',
    marginTop:10}}>
  <Text style={{ fontWeight:700,fontSize:20,marginTop:0,color:'black', }}>Bike Features </Text>
  </View>
...
<DataTable style={styles.container}>
      <DataTable.Header style={styles.tableHeader}>
        <DataTable.Title style = {{marginRight:0,marginLeft:0}}>Feature  Image</DataTable.Title>
        <DataTable.Title style = {{marginRight:0,marginLeft:0}}>Feature  Name</DataTable.Title>
   ...
   />
        ...
        <DataTable.Cell  style={{ marginLeft:70 ,}}>Test Feature  </DataTable.Cell>
        <DataTable.Cell  style={{ marginLeft:70 ,}}> <Link to={{ screen: '', params: { id: '' } }}  style={{}}>
        <Text style={{fontSize:15,
        fontWeight:"600", color:'black'}}
        >    Access Disk/Alloy </Text>  
        </Link>  </DataTable.Cell>
